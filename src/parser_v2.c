...
void print_ast(ASTNode* node, int depth) {
    ...
    const char* type_names[] = {
        ...
        "REPEAT", "GROUP", "SPECIAL_SEQUENCE", "CHAR_RANGE", "DIRECTIVE",
        "QUANTIFIER"
    };
...
ASTNode* parse_primary(Parser* parser) {
    ...
    switch (parser->current_token.type) {
        ...
        case TOKEN_HEX_CHAR: {
            // Detect char range: HEX_CHAR .. HEX_CHAR
            char* start_val = strdup(parser->current_token.value);
            // verifica no input se há '..' após este HEX_CHAR (pulando espaços)
            const char* inp = parser->lexer->input;
            int pos = parser->lexer->position; // posição já após o token atual
            while (inp[pos] != '\0' && isspace((unsigned char)inp[pos])) pos++;
            if (inp[pos] == '.' && inp[pos+1] == '.') {
                // Consume first HEX_CHAR (the current one)
                parser_advance(parser);
                // We expect TOKEN_DOT_DOT provided by the lexer
                if (parser->current_token.type == TOKEN_DOT_DOT) {
                    ...
                    if (parser->current_token.type == TOKEN_HEX_CHAR) {
                        ...
                        add_child_to_node(range_node, create_ast_node(NODE_HEX_CHAR, end_val,
                                                                     parser->current_token.line,
                                                                     parser->current_token.column));
                        parser_advance(parser); // consumes final HEX_CHAR
                        free(start_val);
                        free(end_val);
                        return range_node;
                    } else {
                        printf("Error: Expected HEX_CHAR after '..' in char range at line %d, column %d\n",
                               parser->current_token.line, parser->current_token.column);
                        free(start_val);
                        return NULL;
                    }
                } else {
                    // Something unexpected: lexer didn't produce DOT_DOT; reverts to plain form
                    // (we don't go any further, but we already consumed HEX_CHAR first; so we return a plain HEX_CHAR)
                    ASTNode* simple = create_ast_node(NODE_HEX_CHAR, start_val, line, column);
                    free(start_val);
                    return simple;
                }
            } else {
                // Not a range => returns simple HEX_CHAR
                parser_advance(parser);
        ...
        default:
            // apagar value nulo com segurança ao imprimir se necessário
...
ASTNode* parse_special_sequence(Parser* parser) {
    ...
    while (parser->current_token.type != TOKEN_QUESTION &&
        ...
        if (parser->current_token.type == TOKEN_STRING) {
            add_child_to_node(seq_node,
                create_ast_node(NODE_TERMINAL, parser->current_token.value,
                              parser->current_token.line,
                              parser->current_token.column));
    ...
    if (!parser_expect(parser, TOKEN_QUESTION, "Expected '?'")) {
...
ASTNode* parse_directive(Parser* parser) {
    ...
    if (parser->current_token.type == TOKEN_L_PAREN) {
        parser_advance(parser);
        if (parser->current_token.type == TOKEN_STRING ||
            parser->current_token.type == TOKEN_IDENTIFIER ||
            parser->current_token.type == TOKEN_INTEGER ||
            parser->current_token.type == TOKEN_HEX_CHAR) {
            add_child_to_node(directive_node,
                create_ast_node(NODE_TERMINAL, parser->current_token.value,
                              parser->current_token.line,
                              parser->current_token.column));
...
int main(int argc, char *argv[]) {
    ...
    const char* input =
        "(* A commen. *)\n"
        "term = factor , { factor } ;\n"
        "factor = identifier | \"(\" , expression , \")\" | [ \"-\" ] , number | STRING ;\n"
        "number = digit , { digit } ;\n"
        "digit = #30 .. #39 ; ;\n"
    ...
    if (syntax_tree) {
        ...
        free_ast_node(syntax_tree);
    } else {
        printf("Failed to parse syntax\n");
    }
